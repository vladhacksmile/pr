...
public class Collection {
    ...
    public String head(){
        if(!isEmpty()) {
            ...
            return "Невозможно вывести элемент, так как коллекция пуста!";
        }
    }
    ...
    public void sum_of_annual_turnover() {
        if(!isEmpty()) {
            ...
            System.err.println("Невозможно вывести сумму годового оборота, так как коллекция пуста!");
        }
    }
     ...
     * @param bufferedReader сканнер
     */
    public void remove_greater(BufferedReader bufferedReader) {
        if(!isEmpty()){
     ...
     * @param bufferedReader сканнер
     */
    public void remove_lower(BufferedReader bufferedReader) {
        if(!isEmpty()) {
            int comparable = ConsoleEvent.getAnnualTurnover(bufferedReader);
            int sizeBefore = collection.size();
            collection.removeIf(p -> p.compareAnnualTurnover(comparable) == -1);
