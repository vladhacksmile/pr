...
public class Collection {
    ...
    public String remove_any_by_type(String type) {
        if (!isEmpty()) {
            if (collection.removeIf(p -> p.getType() == Utils.StrToType(type))) return "Элемент удален (" + Utils.StrToType(type) + ")";
    ...
    public String remove_by_id(int id) {
        if(!isEmpty()) {
            if (id > 0) {
                if (collection.removeIf(p -> p.getId() == id)) return "Элемент удален (" + id + ")";
            } else {
     ...
     */
    public String remove_lower(int value) {
        if(!isEmpty()) {
            int sizeBefore = collection.size();
            collection.removeIf(p -> p.compareAnnualTurnover(value) == -1);
            int calc = sizeBefore - collection.size();
